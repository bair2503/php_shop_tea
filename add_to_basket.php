...
if($result->num_rows>0){
    ...
    $query='UPDATE basket SET col_product=col_product+1 WHERE id='.$row[0];
}
else
{
 $query='INSERT INTO basket (user_id, id_product, col_product, weight) VALUES ("'.$_COOKIE['user_id'].'", '.$id.',1,'.$weight.')';
}
$result=$link->query($query);
